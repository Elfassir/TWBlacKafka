...
import com.homeadvisor.kafdrop.service.MessageCreator;
...
public class MessageController
{
   ...
   /**
    * Human friendly view of adding message to specific topic.
    * @param topicName Name of topic
    * @param messageForm Message form for submitting requests to view messages.
    * @param errors
    * @param model
    * @return View for seeing messages in a partition.
    */
   @RequestMapping(method = RequestMethod.GET, value = "/topic/{name:.+}/addMessage")
   public String viewAddMessageForm(@PathVariable("name") String topicName,
                                 @Valid @ModelAttribute("messageForm") MessageVO messageForm,
                                 BindingResult errors,
                                 Model model)
   {
	     ...
	     if (messageForm.isEmpty())
	      {
	         final MessageVO defaultMsg = new MessageVO();
	         model.addAttribute("messageForm", defaultMsg);
	      }
	      ...
	      if (!messageForm.isEmpty() && !errors.hasErrors())
	      {
	         model.addAttribute("messagesResult", messageCreator.addMessageToTopic(topicName,messageForm.getMessage()));
	      }
